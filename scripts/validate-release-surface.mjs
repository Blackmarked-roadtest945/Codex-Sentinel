...
function walk(rootPath, matcher) {
  ...
  for (const entry of entries) {
    if (entry.name === "evals" && rootPath === repoRoot) {
      const evalEntries = readdirSync(path.join(rootPath, entry.name), { withFileTypes: true });
      for (const evalEntry of evalEntries) {
        const evalPath = path.join(rootPath, entry.name, evalEntry.name);
        if (evalEntry.isDirectory()) {
          if (evalEntry.name === "artifacts") {
            continue;
          }
          matches.push(...walk(evalPath, matcher));
          continue;
        }
        if (matcher(evalPath)) {
          matches.push(evalPath);
        }
      }
      continue;
    }
    ...
    if (entry.isDirectory()) {
      ...
      matches.push(...walk(path.join(rootPath, entry.name), matcher));
      continue;
    }
