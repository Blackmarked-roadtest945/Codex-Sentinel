# Codex Sentinel Eval Scaffold

This directory contains a lightweight runnable scaffold for validating core `codex-sentinel` contract behavior with `codex exec --json`.
...
```bash
node evals/run-codex-sentinel.mjs repo-review-offer explicit-no-persistence
```

## List Available Cases

```bash
node evals/run-codex-sentinel.mjs --list
```
...
- Use `CODEX_SENTINEL_EVAL_TIMEOUT_MS` to raise the per-case timeout when local environment latency is high.
- Inspect the trace artifacts to confirm the case read the repo-scoped `.agents/skills/codex-sentinel/` path rather than a legacy user-installed copy.
