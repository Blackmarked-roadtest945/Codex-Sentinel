...
test("reports unsupported symlinks in the source release surface", () => {
  const releaseRoot = mkdtempSync(path.join(tmpdir(), "codex-sentinel-surface-symlink-"));

  mkdirSync(path.join(releaseRoot, "evals"), { recursive: true });
  writeFileSync(path.join(releaseRoot, "README.md"), "# Codex-Sentinel\n", "utf8");
...
test("fails verification when a source-tree symlink is hidden by a Linux-style zip archive", () => {
  const { tempRoot, tempRepo } = copyRepoToTemp("codex-sentinel-surface-symlink-archive-");
  ...
  try {
    ...
    mkdirSync(path.join(archiveRoot, "Codex-Sentinel"), { recursive: true });
    cpSync(tempRepo, path.join(archiveRoot, "Codex-Sentinel"), { recursive: true });

    const zipResult = spawnSync("zip", ["-qr", archivePath, "Codex-Sentinel"], {
      cwd: archiveRoot,
      encoding: "utf8",
    });
