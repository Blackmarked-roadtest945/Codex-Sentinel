...
This fixture exists to validate full-lifecycle `codex-sentinel` behavior in a workspace that provides repo-local checkpoint guidance.
...
1. `Use $codex-sentinel while we plan a new ASP.NET Core admin dashboard with role-based access.`
...
- Codex Sentinel treats the change as a risky implementation scope.
...
- Codex Sentinel treats the prompt as a risky implementation scope with explicit active-analysis consent.
...
- Prompt 1 should activate `security-plan-gap` automatically.
